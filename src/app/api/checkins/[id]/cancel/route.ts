...
import { cancelInternalCheckin } from "@/lib/server/checkins/internal-service";
...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  try {
    ...
    const body = await parseJsonBody(request, cancellationSchema);
    const item = await cancelInternalCheckin({
