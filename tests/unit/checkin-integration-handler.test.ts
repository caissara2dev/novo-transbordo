...
describe("versioned check-in integration handler", () => {
  ...
  it("returns the explicit integration-disabled code while mode is off", async () => {
    ...
    const response = await handleCheckinPreRegistration(
      ...
      dependencies({ environment: { CHECKIN_INTEGRATION_MODE: "off" } })
    );
    ...
    expect(await response.json()).toEqual({
      ...
      error: {
        ...
        message: "Integração de check-in indisponível."
      }
    });
  });
  ...
  it("resolves the current version server-side before confirming in the system", async () => {
